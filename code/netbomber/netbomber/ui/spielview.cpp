...
: QWidget(parent,name), _player(NULL)
{
  ...
  _guiinterface = PDToGUIInterface::instance();
  _guiinterface->setSpielfeld(_spielfeld);
	
}
SpielView::~SpielView()
{

  _pdinterface->release();
  _guiinterface->setSpielfeld(NULL);
  _guiinterface->release();
  delete _spielfeld;
  delete _gamecanvas;
  delete _playerview;
  delete _spieloptview;
  delete _serverview;
  delete _joinserverview;
  delete _menubar;
  delete _networkmenu;
